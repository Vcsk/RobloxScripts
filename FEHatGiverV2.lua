...
Credits to Fedoratum
Modified by !vcsk0#1516

]]

function mOut(txt, type, cooldown)
    if type == 1 then
        spawn(function()
...
function Shrinkname()
    Punish.FocusLost:connect(function()
        for i,v in pairs(game.Players:GetChildren()) do
            if (string.sub(string.lower(v.Name),1,string.len(Punish.Text))) == string.lower(Punish.Text) or (string.sub(string.lower(v.DisplayName),1,string.len(Punish.Text))) == string.lower(Punish.Text) then
                Punish.Text = v.Name
            end
        end
    end)
...
function Shrinkhat()
    SilentCMD.FocusLost:connect(function()
        for i,v in pairs(game.Players.LocalPlayer.Character:GetDescendants()) do
            if v:IsA("Accessory") then
                if (string.sub(string.lower(v.Name),1,string.len(SilentCMD.Text))) == string.lower(SilentCMD.Text) then
                    SilentCMD.Text = v.Name
                end
            end
        end
    end)
...
mOut("Fedoratum Hat Giver Has Loaded", 2, 3)
wait(3)
mOut("Type '/console' in the chat to see you're hats name", 2, 5)
